...
class BaseZjuClient:
    ...
    def _login_cas(self, login_url: str):
        ...
        if "用户名或密码错误" in text or "异常登录" in text:
            raise RuntimeError("统一身份认证登录失败：用户名或密码错误。")
        if "账号被锁定" in text:
            raise RuntimeError("统一身份认证登录失败：账号已被锁定。")

    def _ensure_courses_session(self):
        if not self._has_cookie("iPlanetDirectoryPro"):
            raise RuntimeError("统一身份认证会话缺失，无法获取学在浙大任务。")
        next_url = self.COURSES_INDEX_URL
        seen: set[str] = set()
        for _ in range(16):
            resp = self.session.get(next_url, timeout=self.timeout, allow_redirects=False)
            ...
            if resp.status_code >= 400:
                resp.raise_for_status()
            if self._has_cookie("session", "courses.zju.edu.cn"):
                return
            break
        if not self._has_cookie("session", "courses.zju.edu.cn"):
            raise RuntimeError("未能获取学在浙大学习平台 session。")

    def _has_cookie(self, name: str, domain_contains: str = "") -> bool:
...
class UndergraduateClient(BaseZjuClient):
    ...
    def get_classes(self, academic_year: str, term: int) -> list[dict[str, Any]]:
        resp = self.session.post(
            self.CLASS_URL,
            data={"xn": academic_year, "xq": self._class_term_query(term), "xh": self.username},
            timeout=self.timeout,
        )
        resp.raise_for_status()
        payload = resp.json()
        class_list = ((payload or {}).get("data") or {}).get("kblist") or []
        result = []
        for item in class_list:
            parsed = self._parse_class_item(item)
            if parsed:
    ...
    def get_exams(self, academic_year: str, exam_term: int) -> list[dict[str, Any]]:
        resp = self.session.post(
            self.EXAM_URL,
            data={"xn": academic_year, "xq": self._exam_term_query(exam_term), "xh": self.username},
            timeout=self.timeout,
        )
        resp.raise_for_status()
        payload = resp.json()
        exam_list = (((payload or {}).get("data") or {}).get("list")) or []
        result: list[dict[str, Any]] = []
        for item in exam_list:
            result.extend(self._parse_exam_item(item))
        return result
    ...
    def _exam_term_query(self, term: int) -> str:
        return "1" if term == EXAM_AUTUMN_WINTER else "2"

    def _parse_class_item(self, item: dict[str, Any]) -> dict[str, Any] | None:
        day_number_raw = item.get("xqj")
        if day_number_raw is None:
            return None
        try:
            day_number = int(str(day_number_raw).strip('"'))
        except Exception:
            return None

        periods = sorted(int(x) for x in (item.get("jc") or []) if str(x).isdigit())
        if not periods:
            return None

        term_arrangements = self._parse_term_arrangements(str(item.get("xq", "")))
        if not term_arrangements:
            return None

        week_arrangement_raw = str(item.get("dsz", "")).strip()
        week_arrangement = WEEK_NORMAL
        if week_arrangement_raw == "0":
            week_arrangement = WEEK_ODD
        elif week_arrangement_raw == "1":
            week_arrangement = WEEK_EVEN

        week_numbers = self._parse_week_numbers(item)

        return {
            "id": hashlib.sha1(json.dumps(item, ensure_ascii=False, sort_keys=True).encode("utf-8")).hexdigest(),
            "name": str(item.get("mc", "")).strip(),
            "teacher": str(item.get("jsxm", "")).strip(),
            "location": str(item.get("skdd", "")).strip(),
            "course_code": str(item.get("kcdm", "")).strip(),
    ...
    def _parse_term_arrangements(self, text: str) -> list[int]:
        result = []
        if "秋" in text:
            result.append(TERM_AUTUMN)
        if "冬" in text:
            result.append(TERM_WINTER)
        if "春" in text:
            result.append(TERM_SPRING)
        if "夏" in text:
            result.append(TERM_SUMMER)
        return result
    ...
    def _parse_week_numbers(self, item: dict[str, Any]) -> list[int]:
        ...
        for start_key, end_key in paired_keys:
    ...
    @classmethod
    def _parse_week_numbers_text(cls, value: Any) -> list[int]:
        if value is None:
            return []
        if isinstance(value, (list, tuple, set)):
            numbers = {
                number
                for item in value
                for number in cls._parse_week_numbers_text(item)
            }
    ...
    def _parse_exam_item(self, item: dict[str, Any]) -> list[dict[str, Any]]:
        result = []
        mapping = (
            ("qmkssj", "qmksdd", "zwxh", "期末考试"),
            ("qzkssj", "qzksdd", "qzzwxh", "期中考试"),
        )
        for agenda_key, location_key, seat_key, label in mapping:
            agenda = str(item.get(agenda_key, "")).strip()
            parsed = self._parse_exam_agenda(agenda)
            if not parsed:
                continue
            start_dt, end_dt = parsed
            description = ""
            if item.get(seat_key):
                description = f"座位号：{item.get(seat_key)}"
            result.append(
                {
                    "id": hashlib.sha1(
                        f"{item.get('kcmc','')}|{agenda}|{label}".encode("utf-8", errors="ignore")
                    ).hexdigest(),
                    "name": f"{str(item.get('kcmc', '')).strip()} {label}",
    ...
    def _parse_exam_agenda(self, text: str) -> tuple[datetime, datetime] | None:
        ...
        start_dt = datetime(year, month, day, shour, sminute, tzinfo=tz)
        end_dt = datetime(year, month, day, ehour, eminute, tzinfo=tz)
