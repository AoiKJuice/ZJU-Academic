ZJU Academic 是给 AstrBot 使用的浙大学业助手。它适合放在私人聊天或固定群聊里，用来查询课表、考试、DDL，并在重要时间前提醒你。

它面向日常使用，可以直接用自然语言询问，机器人会按当前配置读取学业数据并回复。
...
学业数据属于私人信息，查询功能默认只给 AstrBot 管理员使用。非管理员询问课表、考试或 DDL 时，机器人不会返回学业数据。
...
1. 填写浙大统一身份认证账号
    ...
    - `password`：统一身份认证密码
2. 检查校历设置
    默认开启 `auto_calendar_enabled`，插件会自动读取 `Xecades/zju-ical-py` 维护的浙大校历配置，里面包含学期时间和调休安排。
    
    人工校历默认关闭。只有在第三方校历暂未发布某个学期时，才建议打开 `manual_calendar_enabled` 并填写人工日期。第三方校历和人工日期同时存在时，以第三方校历为准；人工日期只填充第三方缺失的学期。
